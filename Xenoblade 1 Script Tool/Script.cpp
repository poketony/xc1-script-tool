...
void Script::generateScriptFile(std::string name, const Script* original, RebuildPolicy policy) {
        ...
        auto canReuseSection = [&](SectionType type, bool unchanged) -> bool {
                ...
                if (policy == RebuildPolicy::StringsOnly) {
                        if (type == SectionType::StringPool) return false;
        ...
        auto writePadding = [&](size_t padding) {
                for (size_t i = 0; i < padding; i++) script.put((char)0x0);
        };

        unsigned int alignment = 16;
        if (original != nullptr) alignment = std::max(alignment, original->originalAlignment);
        size_t remainder = fileSize % alignment;
        if (remainder != 0) writePadding(alignment - remainder);
